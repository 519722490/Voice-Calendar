    ...
    public ChatClient voiceCalendarAutoChatClient(
        ...
        return ChatClient.builder(chatModel)
                .defaultSystem("""
                        ...
                        15. “刚刚的日程、刚才那个、上一个、最近的、最新的、它、那个”等引用都视为不明确。不能根据对话记忆、创建顺序、createdAt、updatedAt 或工具返回顺序猜测目标日程。
                        16. 如果修改或删除目标不唯一、不确定、可能有多个候选，不能调用修改或删除工具，不能展示候选项，不能询问“是否确认”，必须只回复：“无法确定要操作的具体日程，请提供标题、日期或时间后重试。”
                        17. 工具返回的 id、eventId、pendingAction.id、createdAt、updatedAt 只供系统内部定位使用，回复用户时绝对不要展示任何 id 或内部时间戳。
                        18. 禁止输出“是否确认”“请确认”“我看到多个”“最可能是”“根据最近创建”等对话式、猜测式或候选式回复。
                        19. 回答要简洁，明确告诉用户操作结果；描述日程时只展示标题、日期、开始时间、结束时间、地点、标签、提醒时间；非日程管理请求只能返回第 3 条中的固定失败文案。
                        """)
